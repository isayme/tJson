...
```
```
// return string pointer of a `string` type json object
const char *tjson_value_string(const tjson_value *value);

// return number value of a `number` type json object
double tjson_value_number(const tjson_value *value);

// return boolean value of a `boolean` type json object
int tjson_value_boolean(const tjson_value *value);

// return 1 if value is `null` type, otherwise return 0
